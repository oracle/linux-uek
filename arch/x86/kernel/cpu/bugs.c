...
static void vmscape_enable_mitigation(void);
...
static void spectre_v2_determine_rsb_fill_type_at_vmexit(enum spectre_v2_mitigation mode)
{
	/*
...
static void activate_spectre_v2_mitigation(enum spectre_v2_mitigation mode, enum spectre_v2_mitigation_cmd cmd)
{
	...
	} else if (ibrs_supported && !spectre_v2_in_ibrs_mode(mode)) {
...
static char *pbrsb_eibrs_state(void)
{
	if (boot_cpu_has_bug(X86_BUG_EIBRS_PBRSB)) {
		if (boot_cpu_has(X86_FEATURE_RSB_VMEXIT_LITE) ||
		    boot_cpu_has(X86_FEATURE_VMEXIT_RSB_FULL))
...
static const char *spectre_bhi_state(void)
{
	...
	else if (retpoline_enabled() && rrsba_disabled)
