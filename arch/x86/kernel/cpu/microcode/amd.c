...
static int __apply_microcode_amd(struct microcode_amd *mc, u32 cur_rev, unsigned int psize)
{
...
void load_ucode_amd_ap(unsigned int cpuid_1_eax)
{
	...
	if (*new_rev && rev <= mc->hdr.patch_id) {
		if (!__apply_microcode_amd(mc, rev, *new_size)) {
			*new_rev = mc->hdr.patch_id;
			return;
		}
	}
...
void reload_ucode_amd(unsigned int cpu)
{
	...
	if (rev < mc->hdr.patch_id) {
		if (!__apply_microcode_amd(mc, rev, ucode_new_size)) {
