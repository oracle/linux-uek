					     ...
					     union kvm_mmu_page_role role)
{
	...
	if (!role.direct) {
		...
		if (role.level > PT_PAGE_TABLE_LEVEL && need_sync)
...
static union kvm_mmu_page_role kvm_mmu_child_role(u64 *sptep, bool direct,
						  unsigned int access)
{
	struct kvm_mmu_page *parent_sp = page_header(__pa(sptep));
	union kvm_mmu_page_role role;
	...
	if (!role.gpte_is_8_bytes) {
		WARN_ON_ONCE(role.level != PT_PAGE_TABLE_LEVEL);
...
static struct kvm_mmu_page *kvm_mmu_get_child_sp(struct kvm_vcpu *vcpu,
						 ...
						 unsigned int access)
{
	union kvm_mmu_page_role role = kvm_mmu_child_role(sptep, direct,
							  access);
...
static void __link_shadow_page(struct kvm_vcpu *vcpu, u64 *sptep,
			       struct kvm_mmu_page *sp)
{
	u64 spte;
...
static void link_shadow_page(struct kvm_vcpu *vcpu, u64 *sptep,
			     struct kvm_mmu_page *sp)
{
	__link_shadow_page(vcpu, sptep, sp);
}
