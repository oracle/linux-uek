...
{
	return __skip_emulated_instruction(vcpu, true);
}
...
static int svm_update_soft_interrupt_rip(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	if (!__skip_emulated_instruction(vcpu, !nrips))
		return -EIO;
...
static void svm_queue_exception(struct kvm_vcpu *vcpu)
{
	...
	if (kvm_exception_is_soft(nr) &&
	    svm_update_soft_interrupt_rip(&svm->vcpu))
		return;
...
static int svm_create_vcpu(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	if (irqchip_in_kernel(svm->vcpu.kvm) && kvm_apicv_activated(svm->vcpu.kvm))
...
static void svm_set_irq(struct kvm_vcpu *vcpu, bool reinjected)
{
	struct vcpu_svm *svm = to_svm(vcpu);
	u32 type;
...
static void svm_complete_interrupts(struct vcpu_svm *svm)
{
	...
	if (soft_int_injected)
		svm_complete_soft_interrupt(&svm->vcpu, vector, type);
	...
	case SVM_EXITINTINFO_TYPE_SOFT:
		kvm_queue_interrupt(&svm->vcpu, vector, true);
		break;
	default:
		break;
	}
