...
static int fbt_brk_hook(struct pt_regs *regs, unsigned int esr)
{
	...
	fbt_probe_t	*fbt = fbt_probetab[FBT_ADDR2NDX(ip)];

	for (; fbt != NULL; fbt = fbt->fbp_hashnext) {
		if ((uintptr_t)fbt->fbp_patchpoint == ip) {
			if (fbt->fbp_isret) {
				/* not implemented yet */
			} else {
				...
				dtrace_probe(fbt->fbp_id, regs->regs[0],
...
{
	fbp->fbp_patchval = BRK64_OPCODE_DPROBE_FBT;
	fbp->fbp_savedval = dtrace_text_peek(fbp->fbp_patchpoint);
	fbp->fbp_isret = (type == FBT_RETURN);
}
...
static struct break_hook dtrace_fbt_break_hook = {
	.esr_mask = BRK64_ESR_MASK,
	.esr_val = BRK64_ESR_DPROBES_FBT,
	.fn = fbt_brk_hook,
};
...
int fbt_dev_init_arch(void)
{
	...
	dtrace_brk_start(&dtrace_fbt_break_hook);
...
{
	dtrace_brk_stop(&dtrace_fbt_break_hook);
