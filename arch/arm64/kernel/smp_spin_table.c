...
#if defined(CONFIG_PENSANDO_SOC_PCIE)
	/*
	 * Ask kpcimgr if it would like to hijack a cpu. It
	 * will return its polling function address for the
	 * cpu it's borrowing, and for all the others, it
	 * will return spin_table_start.
	 */
	entry = kpcimgr_get_entry(spin_table_start, cpu);
	cpu_soft_restart(1, entry, spin_table_start, 0, cpu);
#else
	cpu_soft_restart(1, spin_table_start, 0, 0, cpu);
#endif
