...
{
	hvirq_major = simple_strtoul(p, NULL, 10);
	return 0;
}
...
/* This function for the timer interrupt.
 */
...
static int __init early_num_ivecs(char *p)
{
	...
	count = simple_strtoul(p, NULL, 0);

	if (!count || (count > MAX_NUM_IVECS))
		pr_info("Defaulted NUM_IVECS to %d.\n", DEFAULT_NUM_IVECS);
	else
		nr_ivec = count;

	return 0;
}
early_param("num_ivecs", early_num_ivecs);
...
struct irq_handler_data {
	...
	/* This probably isn't need for sun4v.
	 * Should be yanked when code is cleaned up.
	 */
	unsigned long	iclr;
	unsigned long	imap;
};
...
unsigned int irq_alloc(unsigned int dev_handle, unsigned int dev_ino)
{
	...
	irq = __irq_alloc_descs(-1, 1, 1, -1, NULL);
		...
		struct irq_chip *chip)
{
	...
	/* This was brought over from legacy. It doesn't
	 *  appear required for sun4v.
	 */
	data->imap = ~0UL;
	data->iclr = ~0UL;
out:
...
unsigned int sun4v_build_virq(u32 devhandle, unsigned int devino)
{
	...
	/* This is borrowed from the original function - see below.
	 */
...
static void __init irq_ivector_init(void)
{
	 ...
	 * only virq delivery. Thus we are assuming children function too.
	 */
	if (hv_irq_version >= 3)
		return;
