				 ...
				 void *buf, int len)
{
	...
	pr_alert("ds-%llu: Shutdown request from LDOM manager received.\n",
		dp->id);
			      ...
			      void *buf, int len)
{
	...
	pr_alert("ds-%llu: Panic request from LDOM manager received.\n",
		dp->id);
...
void ldom_set_var(const char *var, const char *value)
{
	...
	if (cp) {
		...
		if (ds_var_doorbell == 0 ||
			...
			pr_err("ds-%llu: var-config [%s:%s] failed, response(%d).\n",
			       dp->id, var, value, ds_var_response);
	} else {
		pr_err(PFX "var-config not registered so could not set (%s) variable to (%s).\n",
		       var, value);
	}
}
