 ...
 * Returns: The shift into that pointer to the star to of the slot
 */
 ...
 * @parent: The parent pointer cast as an unsigned long.
 * Returns: The slot mask for that parent.
 */
 ...
 * Returns: The node->parent maple_type
 */
static inline
enum maple_type mas_parent_enum(struct ma_state *mas, struct maple_enode *enode)
{
	...
	/* Dead nodes can reach here as enode->parent = node */
	BUG_ON(ma_mnode_ptr(mte_to_node(enode)->parent) != mte_to_node(enode));
	return 0;
}
...
static void mas_ascend(struct ma_state *mas)
{
	...
	do {
		...
		/* Dead node must be handled at a higher level. */
...
static unsigned long mas_leaf_max_gap(struct ma_state *mas)
{
	...
	for (; i <= max_piv; i++) {
		...
		pstart = pivots[i - 1];
		/* The end cannot be a gap, so this is beyond the data. */
		if (!pstart || pstart == mas->max)
			break;
				    ...
				    void *entry, unsigned char end)
{
	...
	contents = mas_slot_locked(mas, ma_slots(node, mt), slot);
	if (piv + 1 < mas->index) {
