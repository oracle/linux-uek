				 ...
				 unsigned long hugepage_size)
{
	...
	if (hugepage_size != BYTE_LENTH_IN_2M) {
		if (fd >= 0)
			ksft_exit_fail_msg("unexpected memfd_create success with 1G\n");
		else if (errno != EINVAL)
			ksft_exit_fail_msg("unexpected memfd_create error with 1G: %s (%d)\n",
					   strerror(errno), errno);
		else {
			ksft_print_msg("1G pagesize returned EINVAL as expected\n");
			return -1;
		}
	}

	if (fd < 0)
		ksft_exit_fail_msg("Failed to memfd_create: %s (%d)\n",
				   strerror(errno), errno);

	memset(file_stat, 0, sizeof(*file_stat));
	if (fstatfs(fd, file_stat)) {
		close(fd);
		ksft_exit_fail_msg("Failed to fstatfs: %s (%d)\n",
				   strerror(errno), errno);
	}
...
static void *do_hwpoison(void *hwpoison_addr)
{
	...
	if (madvise(hwpoison_addr, hwpoison_size, MADV_HWPOISON) < 0)
		ksft_exit_fail_msg("Failed to MADV_HWPOISON: %s (%d)\n",
				   strerror(errno), errno);
					 ...
					 unsigned long hugepage_size)
{
	...
	for (i = 0; i < offsets_count; ++i) {
		...
		if (ret)
			ksft_exit_fail_msg("Failed to create hwpoison thread: %s (%d)\n",
					   strerror(errno), errno);
			     ...
			     unsigned long *nr_hugepages)
{
	...
	if (cmdfile == NULL) {
		ksft_print_msg(EPREFIX "failed to popen nr_hugepages: %s (%d)\n",
			       strerror(errno), errno);
		return -1;
	}

	if (!fgets(buffer, sizeof(buffer), cmdfile)) {
		ksft_print_msg(EPREFIX "failed to read nr_hugepages: %s (%d)\n",
			       strerror(errno), errno);
		pclose(cmdfile);
		return -1;
	}
...
static void test_main(int fd, unsigned long hugepage_size)
{
	...
	if (ftruncate(fd, len) < 0)
		ksft_exit_fail_msg("Failed to ftruncate: %s (%d)\n",
				   strerror(errno), errno);
	...
	if (map == MAP_FAILED)
		ksft_exit_fail_msg("Failed to mmap: %s (%d)\n",
				   strerror(errno), errno);
	...
	if (ftruncate(fd, 0) < 0)
		ksft_exit_fail_msg("Failed to ftruncate to 0: %s (%d)\n",
				   strerror(errno), errno);
	munmap(map, len);
	close(fd);
}
