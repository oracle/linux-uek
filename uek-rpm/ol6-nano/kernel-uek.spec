...
%endif
%define pkg_release 1%{?dist}ueknano%{?buildid}
...
Requires(pre): linux-nano-firmware >= 20160909-47.gitc883a6b6.0.4\
...
%define variant %{?build_variant:%{build_variant}}%{!?build_variant:-ueknano}
Name: kernel%{?variant}
...
%ifarch x86_64
%kernel_variant_post -u -v ueknano -r (kernel%{variant}|kernel%{variant}-debug|kernel-ovs)
%else
%kernel_variant_post -u -v ueknano -r (kernel%{variant}|kernel%{variant}-debug|kernel-ovs)
%endif
