...
skip:
		...
		if ((wake_thread = should_wake_printk_thread(start_ns)) ||
			 ...
			 * 1. We will awaken the kthread which will come back
			 *    and handle the rest of the messages.
			 * 2. We *are* the kthread, but our time slice is
			 *    expired. We drop the lock and return, but will
			 *    remain in a RUNNABLE state. By dropping the lock,
			 *    we allow other tasks in printk() to continue the
			 *    work, and if they run too long, they'll wake us
			 *    up.
			 */
			console_locked = 0;
			up_console_sem();
