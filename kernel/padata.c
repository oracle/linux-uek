...
/**
 * __padata_do_multithreaded - run a multithreaded job
...
static int padata_cpu_prepare(unsigned int cpu, struct hlist_node *node)
{
	struct padata_instance *pinst;
	int ret;

	pinst = hlist_entry_safe(node, struct padata_instance, cpu_prep_node);
	if (!pinst_has_cpu(pinst, cpu))
		return 0;
...
static enum cpuhp_state hp_prepare;
#endif

static void __padata_free(struct padata_instance *pinst)
{
#ifdef CONFIG_HOTPLUG_CPU
	cpuhp_state_remove_instance_nocalls(hp_prepare, &pinst->cpu_prep_node);
					    ...
					    const struct cpumask *cbcpumask)
{
	...
	cpuhp_state_add_instance_nocalls_cpuslocked(hp_prepare,
						    &pinst->cpu_prep_node);
#endif
...
void __init padata_init(void)
{
	...
	ret = cpuhp_setup_state_multi(CPUHP_BP_PREPARE_DYN, "padata:prepare",
				      NULL, padata_cpu_prepare);
	if (ret < 0)
...
remove_prepare_state:
#ifdef CONFIG_HOTPLUG_CPU
	cpuhp_remove_multi_state(hp_prepare);
remove_online_state:
