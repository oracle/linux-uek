...
lock:
		...
		if (cavium_ipsec_process) {
			 ...
			 * but we require skb->data to point to the start of
			 * ip header.
			 */
			skb_push(skb, (unsigned int)((uint64_t)skb->data -
						     (uint64_t)ip_hdr(skb)));
			...
			nexthdr = cavium_ipsec_process(x, skb, offset,
						       0 /*DECRYPT*/);
