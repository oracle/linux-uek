...
struct devlink {
	...
	UEK_KABI_FILL_HOLE(struct rcu_head rcu)
	/*
	 * rcu_head is 16 bytes, so we need to occupy two reserved spots.
	 */
	UEK_KABI_FILL_HOLE(struct notifier_block netdevice_nb)
	/*
	 * netdevice_nb is 24 bytes, so we need to occupy two reserved
	 * spots plus an additional 8 bytes memory hole below.
	 */
