...
- The memory pages affected by UE have a large smallest granularity, for
  example 2M hugepage, but the actual corrupted amount of the page is only
...
Memory failure recovery for 2M HugeTLB is a good example. With memfd
...
- UEK7 looses the entire UE impacted hugepage once the memfd is closed.

- UEK7 doesn't report an already poisoned sub-page inside a
  ``MFD_MF_KEEP_UE_MAPPED`` hugepage as being already poisoned.

- UEK7 currently only supports 2M hugepages, so if a different size is
  given with ``MFD_HUGETLB`` or if no size is given and the default
  Hugepagesize is larger than 2M, ``memfd_create`` returns EINVAL.
