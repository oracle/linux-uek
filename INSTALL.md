...
## Setting UEK-next as default kernel
  ...
  - Post installation, set it as the default using `grubby` as:
    ...
    ```
	sudo grubby --set-default=/boot/vmlinuz-6.17.0-0.el9ueknext.$(name -p)
	```
...
Add a new repository using DNF:

```
sudo dnf config-manager --add-repo 'https://yum.oracle.com/repo/OracleLinux/OL$releasever/developer/UEKnext/$basearch'
```

Or manually, create a file in `/etc/yum.repos.d` such as `/etc/yum.repos.d/uek-next-developer.repo`, with the following content:


```
[ol9_developer_UEKnext]
name=UEK-next Developer Release
baseurl=https://yum$ociregion.$ocidomain/repo/OracleLinux/OL$releasever/developer/UEKnext/$basearch/
