...
struct bpf_map {
	...
	/* When taking upstream changes we got some space that we could
	 * reuse in future, so using UEK_KABI_RESERVE()
	 */
	UEK_KABI_RESERVE(1)
#endif
	bool bypass_spec_v1;
	...
	UEK_KABI_USE(1, u64 cookie) /* write-once */
};
...
struct bpf_prog_aux {
	...
	UEK_KABI_FILL_HOLE(bool changes_pkt_data)
