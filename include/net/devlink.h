...
struct devlink_port_pci_sf_attrs {
	/* KABI note: entire struct size should not exceed 12 bytes */
	u32 controller;
	u32 sf;
	u16 pf;
	u8 external:1;
...
struct devlink_port_attrs {
	...
	union {
		...
		UEK_KABI_EXTEND(struct devlink_port_pci_sf_attrs pci_sf)
	};
};
...
struct devlink_trap_group {
	...
	u16 id;
	bool generic;
	UEK_KABI_FILL_HOLE(u32 init_policer_id)
};
...
struct devlink_trap {
	...
	u16 id;
	const char *name;
	UEK_KABI_REPLACE(struct devlink_trap_group group, u16 init_group_id)
	u32 metadata_cap;
};
...
struct devlink_ops {
	 ...
	 */
	UEK_KABI_EXTEND(int (*trap_policer_init)(struct devlink *devlink,
					 const struct devlink_trap_policer *policer))
	/**
	 ...
	 */
	UEK_KABI_EXTEND(void (*trap_policer_fini)(struct devlink *devlink,
					  const struct devlink_trap_policer *policer))
	/**
	 * @trap_policer_set: Trap policer parameters set function.
	 */
	UEK_KABI_EXTEND(int (*trap_policer_set)(struct devlink *devlink,
					const struct devlink_trap_policer *policer,
					u64 rate, u64 burst,
					struct netlink_ext_ack *extack))
	/**
	 ...
	 * by the policer.
	 */
	UEK_KABI_EXTEND(int (*trap_policer_counter_get)(struct devlink *devlink,
						const struct devlink_trap_policer *policer,
						u64 *p_drops))
	/**
	 ...
	 */
	UEK_KABI_EXTEND(int (*port_new)(struct devlink *devlink,
	 ...
	 * Return: 0 on success, negative value otherwise.
	 */
	UEK_KABI_EXTEND(int (*port_del)(struct devlink *devlink,
					unsigned int port_index,
					struct netlink_ext_ack *extack))
	/**
	 ...
	 * Return: 0 on success, negative value otherwise.
	 */
	UEK_KABI_EXTEND(int (*port_fn_state_get)(struct devlink *devlink,
						 struct devlink_port *port,
	 ...
	 * Return: 0 on success, negative value otherwise.
	 */
	UEK_KABI_EXTEND(int (*port_fn_state_set)(struct devlink *devlink,
						 struct devlink_port *port,
						 enum devlink_port_fn_state state,
						 struct netlink_ext_ack *extack))
	...
	UEK_KABI_EXTEND(int (*rate_node_tx_share_set)(struct devlink_rate *devlink_rate, void *priv,
						      u64 tx_share, struct netlink_ext_ack *extack))
	UEK_KABI_EXTEND(int (*rate_node_tx_max_set)(struct devlink_rate *devlink_rate, void *priv,
						    u64 tx_max, struct netlink_ext_ack *extack))
	UEK_KABI_EXTEND(int (*rate_node_new)(struct devlink_rate *rate_node, void **priv,
					     struct netlink_ext_ack *extack))
