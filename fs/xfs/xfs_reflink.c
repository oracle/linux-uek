	...
	xfs_fileoff_t		*end_fsb)
{
	...
	/*
	 * Only remap real extent that contain data.  With AIO, speculative
	...
	/* Free the CoW orphan record. */
	error = xfs_refcount_free_cow_extent(mp, &dfops, del.br_startblock,
			del.br_blockcount);
	if (error)
		goto out_defer;

	/* Map the new blocks into the data fork. */
	error = xfs_bmap_map_extent(mp, &dfops, ip, &del);
	if (error)
		goto out_defer;
