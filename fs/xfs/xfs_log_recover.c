	...
	struct list_head		*capture_list)
{
	...
	spin_unlock(&ailp->ail_lock);
	error = xfs_efi_recover(mp, efip, capture_list);
	spin_lock(&ailp->ail_lock);
	...
	struct list_head		*capture_list)
{
	...
	spin_unlock(&ailp->ail_lock);
	error = xfs_rui_recover(mp, ruip, capture_list);
	spin_lock(&ailp->ail_lock);
	...
	struct list_head		*capture_list)
{
	...
	spin_unlock(&ailp->ail_lock);
	error = xfs_cui_recover(mp, cuip, capture_list);
	spin_lock(&ailp->ail_lock);
	...
	struct list_head		*capture_list)
{
	...
	spin_unlock(&ailp->ail_lock);
	error = xfs_bui_recover(mp, buip, capture_list);
	spin_lock(&ailp->ail_lock);
