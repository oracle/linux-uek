					 ...
					 struct btrfs_device **device)
{
	...
	if (srcdevid) {
		ret = 0;
		*device = btrfs_find_device(root->fs_info->fs_devices, srcdevid, NULL,
					    NULL, true);
		if (!*device)
			ret = -ENOENT;
	} else {
			  ...
			  struct btrfs_chunk *chunk)
{
	...
	for (i = 0; i < num_stripes; i++) {
		...
		read_extent_buffer(leaf, uuid, (unsigned long)
				   ...
				   BTRFS_UUID_SIZE);
		map->stripes[i].dev = btrfs_find_device(root->fs_info->fs_devices, devid,
							uuid, NULL, true);
			...
			struct btrfs_dev_item *dev_item)
{
	...
	device = btrfs_find_device(root->fs_info->fs_devices, devid, dev_uuid, fs_uuid, true);
	if (!device) {
		if (!btrfs_test_opt(root, DEGRADED))
			return -EIO;
			...
			struct btrfs_ioctl_get_dev_stats *stats)
{
	...
	mutex_lock(&fs_devices->device_list_mutex);
	dev = btrfs_find_device(root->fs_info->fs_devices, stats->devid, NULL, NULL, true);
	mutex_unlock(&fs_devices->device_list_mutex);
