				  ...
				  u64 *async_transid)
{
	 ...
	 * of create_snapshot().
	 */
	ret = btrfs_subvolume_reserve_metadata(root, &block_rsv, 7);
	if (ret)
		return ret;
					     ...
					     void __user *arg)
{
	 ...
	 * ref/backref.
	 */
	err = btrfs_subvolume_reserve_metadata(root, &block_rsv, 5);
	if (err)
		goto out_up_write;
...
next:
	...
	ret = 0;
	/* 
	 * Truncate Page cache pages so that future reads wil see the cloned
