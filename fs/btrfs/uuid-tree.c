			...
			u64 subid_cpu)
{
	...
	if (ret >= 0) {
		 ...
		 */
		btrfs_extend_item(trans, uuid_root, path, sizeof(subid_le));
		eb = path->nodes[0];
		slot = path->slots[0];
		offset = btrfs_item_ptr_offset(eb, slot);
			...
			u64 subid)
{
	...
	btrfs_truncate_item(trans, uuid_root, path, item_size - sizeof(subid), 1);
...
again_search_slot:
	path->keep_locks = 1;
	ret = btrfs_search_forward(root, &key, &max_key, path, 0, 0);
	if (ret) {
		if (ret > 0)
			ret = 0;
		goto out;
