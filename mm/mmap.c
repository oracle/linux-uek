...
int vma_dup(struct vm_area_struct *old_vma, struct mm_struct *mm)
{
	...
	vm_stat_account(mm, old_vma->vm_flags, NULL, npages);
	...
	*vma = *old_vma;
	 ...
	 * process which includes memory locking, userfaultfd, and
	 * preservation over exec flags.
	 */
