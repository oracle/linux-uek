						  ...
						  struct page *reuse)
{
	...
	if (!pmd)
		return NULL;
	pte = vmemmap_pte_populate(pmd, addr, node, reuse);
	if (!pte)
		return NULL;
					    ...
					    struct page *reuse)
{
	...
	for (; addr < end; addr += PAGE_SIZE) {
		pte = vmemmap_populate_address(addr, node, reuse);
		if (!pte)
			return -ENOMEM;
	}
						     ...
						     struct dev_pagemap *pgmap)
{
	...
	for (addr = start; addr < end; addr += size) {
		...
		pte = vmemmap_populate_address(addr, node, NULL);
		if (!pte)
			return -ENOMEM;
		 ...
		 */
		next += PAGE_SIZE;
		rc = vmemmap_populate_range(next, last, node,
					    pte_page(*pte));
		if (rc)
			return -ENOMEM;
	}
