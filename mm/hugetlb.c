						...
						nodemask_t *node_alloc_noretry)
{
	...
	/*
	 * By default we always try hard to allocate the page with __GFP_REPEAT
	 * flag. However, if we are allocating pages in a loop (to adjust
	 * global huge page counts) and previous allocation failed, do not
	 * continue to try hard on the same node.  Use the node_alloc_noretry
	 * bitmap to manage this state information.
	 */
	...
	page = alloc_pages_exact_node(nid, gfp_mask, huge_page_order(h));
	if (page) {
...
out:
	/*
	 * If we did not specify __GFP_REPEAT, but still got a page this
