...
static void update_and_free_pages_bulk(struct hstate *h, struct list_head *list)
{
	...
	/*
	 * First allocate required vmemmmap (if necessary) for all pagess on
	 * list.  If vmemmap can not be allocated, we can not free page to
	...
	/*
	 * If vmemmmap allocation was performed on any page above, take lock
	 * to clear destructor of all pages on list.  This avoids the need to
	 * lock/unlock for each individual page.
	 * The assumption is vmemmap allocation was performed on all or none
	 * of the pages on the list.  This is true expect in VERY rare cases.
	 */
	if (clear_dtor) {
		spin_lock_irq(&hugetlb_lock);
	...
	/*
	 * Free pagess back to low level allocators.  vmemmap and destructors
	 * were taken care of above, so update_and_free_hugetlb_page will
			     ...
			     unsigned long len)
{
	...
	for (; old_addr < old_end; old_addr += sz, new_addr += sz) {
		...
		if (huge_pmd_unshare(mm, &old_addr_copy, src_pte)) {
			shared_pmd = true;
			continue;
		}
