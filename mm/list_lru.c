...
list_lru_from_memcg_idx(struct list_lru *lru, int nid, int idx)
{
	 ...
	 */
	mlrus = rcu_dereference_check(lru->ext->mlrus, lockdep_is_held(&nlru->lock));
	if (mlrus && idx >= 0)
...
static int memcg_init_list_lru(struct list_lru *lru, bool memcg_aware)
{
	...
	RCU_INIT_POINTER(lru->ext->mlrus, mlrus);
...
static void memcg_destroy_list_lru(struct list_lru *lru)
{
	 ...
	 */
	mlrus = rcu_dereference_protected(lru->ext->mlrus, true);
...
static int memcg_update_list_lru(struct list_lru *lru, int old_size, int new_size)
{
	...
	old = rcu_dereference_protected(lru->ext->mlrus,
	...
	spin_lock_irq(&lru->ext->lock);
	memcpy(&new->mlru, &old->mlru, flex_array_size(new, mlru, old_size));
	rcu_assign_pointer(lru->ext->mlrus, new);
	spin_unlock_irq(&lru->ext->lock);
					 ...
					 int old_size, int new_size)
{
	...
	mlrus = rcu_dereference_protected(lru->ext->mlrus,
					  lockdep_is_held(&list_lrus_mutex));
	/*
				     ...
				     struct list_lru *lru)
{
	...
	rcu_read_lock();
	allocated = !!rcu_dereference(lru->ext->mlrus)->mlru[idx];
	rcu_read_unlock();
			 ...
			 gfp_t gfp)
{
	...
	spin_lock_irqsave(&lru->ext->lock, flags);
	mlrus = rcu_dereference_protected(lru->ext->mlrus, true);
	while (i--) {
	...
	spin_unlock_irqrestore(&lru->ext->lock, flags);
		    ...
		    struct lock_class_key *key, struct shrinker *shrinker)
{
	...
	lru->ext = kcalloc(1, sizeof(struct list_lru_ext), GFP_KERNEL);
	if (!lru->ext)
		goto out;

	lru->ext->node = kcalloc(nr_node_ids, sizeof(*lru->ext->node), GFP_KERNEL);
	if (!lru->ext->node) {
		kfree(lru->ext);
		lru->ext = NULL;
	...
	for_each_node(i) {
		spin_lock_init(&lru->ext->node[i].lock);
		if (key)
			lockdep_set_class(&lru->ext->node[i].lock, key);
		init_one_lru(&lru->ext->node[i].lru);
	}
