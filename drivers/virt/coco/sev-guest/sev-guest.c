...
cmd:
	...
	if (arg->vmm_error == SNP_GUEST_VMM_ERR_INVALID_LEN) {
		...
		if (copy_to_sockptr_offset(io->req_data, 0, req, sizeof(*req)))
			ret = -EFAULT;
	}
	...
	if (npages && copy_to_sockptr_offset(certs_address, 0, snp_dev->certs_data, req->certs_len)) {
		ret = -EFAULT;
		goto e_free;
	}

	if (copy_to_sockptr_offset(io->resp_data, 0, resp, sizeof(*resp)))
		ret = -EFAULT;
