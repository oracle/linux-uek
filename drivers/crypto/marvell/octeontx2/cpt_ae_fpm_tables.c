 ...
 */
#define AE_FPM_P192_LEN 2160
#define AE_FPM_P224_LEN 2160
#define AE_FPM_P256_LEN 2160
#define AE_FPM_P384_LEN 2520
#define AE_FPM_P521_LEN 3240
...
const struct ae_fpm_entry ae_fpm_tbl_scalar[CPT_AE_EC_ID_PMAX] = {
	{
...
{
	uint32_t len_align[CPT_AE_EC_ID_PMAX];
	dma_addr_t dma_addr;
	int i, len = 0;
	uint8_t *data;
	...
	for (i = 0; i < CPT_AE_EC_ID_PMAX; i++) {
		len_align[i] = ALIGN(ae_fpm_tbl_scalar[i].len, 128);
		len += len_align[i];
	}
	...
	dma_addr = dma_map_single(&pdev->dev, data, len, DMA_BIDIRECTIONAL);
	if (unlikely(dma_mapping_error(&pdev->dev, dma_addr))) {
		dev_err(&pdev->dev, "DMA mapping failed for FPM table\n");
		kfree(data);
		return -EIO;
	}
	tbl->fpm_virt = data;
	tbl->fpm_dma_addr = dma_addr;
	tbl->dma_len = len;

	for (i = CPT_AE_EC_ID_P192; i < CPT_AE_EC_ID_PMAX; i++) {
		memcpy(data, ae_fpm_tbl_scalar[i].data,
		       ae_fpm_tbl_scalar[i].len);
		tbl->fpm_iova[i] = cpu_to_be64(dma_addr);
		dma_addr = dma_addr + len_align[i];
		data = data + len_align[i];
	}
...
{
	dma_unmap_single(&pdev->dev, tbl->fpm_dma_addr, tbl->dma_len,
			 DMA_BIDIRECTIONAL);
	kfree(tbl->fpm_virt);
}
