 ...
 */
static void
_mpt2sas_raise_sigio(struct MPT2SAS_ADAPTER *ioc,
	struct SL_WH_TRIGGERS_EVENT_DATA_T *event_data)
{
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: enter\n",
	    ioc->name, __func__));
	...
	if (!mpi_reply)
		goto out;
	mpi_reply->Event = cpu_to_le16(MPI2_EVENT_DIAGNOSTIC_TRIGGER_FIRED);
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: add to driver "
	    "event log\n", ioc->name, __func__));
 ...
 out:
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: clearing "
	    "diag_trigger_active flag\n", ioc->name, __func__));
	ioc->diag_trigger_active = 0;
	spin_unlock_irqrestore(&ioc->diag_trigger_lock, flags);

	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: exit\n", ioc->name,
	    __func__));
}
 ...
 */
void
mpt2sas_process_trigger_data(struct MPT2SAS_ADAPTER *ioc,
	struct SL_WH_TRIGGERS_EVENT_DATA_T *event_data)
{
	u8 issue_reset = 0;
	...
	if ((ioc->diag_buffer_status[MPI2_DIAG_BUF_TYPE_TRACE] &
		...
		dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: release "
		    "trace diag buffer\n", ioc->name, __func__));
 ...
 */
void
mpt2sas_trigger_master(struct MPT2SAS_ADAPTER *ioc, u32 trigger_bitmask)
{
	struct SL_WH_TRIGGERS_EVENT_DATA_T event_data;
	unsigned long flags;
	u8 found_match = 0;
 ...
 by_pass_checks:
	...
	if (ioc->diag_trigger_master.MasterData & trigger_bitmask) {
		...
		dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: setting "
		"diag_trigger_active flag\n", ioc->name, __func__));
	}
	spin_unlock_irqrestore(&ioc->diag_trigger_lock, flags);
	...
	event_data.trigger_type = MPT2SAS_TRIGGER_MASTER;
 ...
 */
void
mpt2sas_trigger_event(struct MPT2SAS_ADAPTER *ioc, u16 event,
	u16 log_entry_qualifier)
{
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: setting "
	    "diag_trigger_active flag\n", ioc->name, __func__));
	memset(&event_data, 0, sizeof(struct SL_WH_TRIGGERS_EVENT_DATA_T));
	event_data.trigger_type = MPT2SAS_TRIGGER_EVENT;
 ...
 */
void
mpt2sas_trigger_scsi(struct MPT2SAS_ADAPTER *ioc, u8 sense_key, u8 asc,
	u8 ascq)
{
	struct SL_WH_TRIGGERS_EVENT_DATA_T event_data;
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: setting "
	    "diag_trigger_active flag\n", ioc->name, __func__));
	memset(&event_data, 0, sizeof(struct SL_WH_TRIGGERS_EVENT_DATA_T));
	event_data.trigger_type = MPT2SAS_TRIGGER_SCSI;
 ...
 */
void
mpt2sas_trigger_mpi(struct MPT2SAS_ADAPTER *ioc, u16 ioc_status, u32 loginfo)
{
	struct SL_WH_TRIGGERS_EVENT_DATA_T event_data;
	...
	dTriggerDiagPrintk(ioc, printk(MPT2SAS_INFO_FMT "%s: setting "
	    "diag_trigger_active flag\n", ioc->name, __func__));
	memset(&event_data, 0, sizeof(struct SL_WH_TRIGGERS_EVENT_DATA_T));
	event_data.trigger_type = MPT2SAS_TRIGGER_MPI;
