...
/**
 * lpfc_debugfs_dumpSLIqinfo_data - Dump host SLIM info to a buffer
 * @phba: The HBA to gather SLI4 queue information from.
 * @buf: The buffer to dump log into.
 * @size: The maximum amount of data to process.
 *
 * Description:
 ...
 **/
static int
lpfc_debugfs_dumpSLIqinfo_data(struct lpfc_hba *phba, char *buf, int size)
{
	int len = 0;
	int x, cnt;
	int max_cnt;
...
too_big:
	len +=  snprintf(buf+len, size-len, "Truncated ...\n");
	spin_unlock_irq(&phba->hbalock);
	return len;
}
...
/**
 * lpfc_debugfs_dumpSLIqinfo_open - Open the Dump Host SLIM debugfs buffer
 * @inode: The inode pointer that contains a vport pointer.
 * @file: The file pointer to attach the log output.
 *
 * Description:
 ...
 **/
static int
lpfc_debugfs_dumpSLIqinfo_open(struct inode *inode, struct file *file)
{
	struct lpfc_hba *phba = inode->i_private;
	struct lpfc_debug *debug;
	int rc = -ENOMEM;
	...
	/* Round to page boundary */
	debug->buffer = kmalloc(LPFC_DUMPSLIQINFO_SIZE, GFP_KERNEL);
	if (!debug->buffer) {
		kfree(debug);
		goto out;
	}

	debug->len = lpfc_debugfs_dumpSLIqinfo_data(phba, debug->buffer,
...
#undef lpfc_debugfs_op_dumpSLIqinfo
static const struct file_operations lpfc_debugfs_op_dumpSLIqinfo = {
	.owner =        THIS_MODULE,
	.open =         lpfc_debugfs_dumpSLIqinfo_open,
	.llseek =       lpfc_debugfs_lseek,
	.read =         lpfc_debugfs_read,
	.release =      lpfc_debugfs_release,
};
...
lpfc_debugfs_initialize(struct lpfc_vport *vport)
{
	...
	if (!phba->hba_debugfs_root) {
		...
		/* Setup dumpSLIqinfo */
		if (phba->sli_rev == LPFC_SLI_REV4) {
			snprintf(name, sizeof(name), "dumpSLIqinfo");
			phba->debug_dumpSLIqinfo =
				debugfs_create_file(name,
					S_IFREG|S_IRUGO|S_IWUSR,
					phba->hba_debugfs_root,
					phba, &lpfc_debugfs_op_dumpSLIqinfo);
			if (!phba->debug_dumpSLIqinfo) {
				lpfc_printf_vlog(vport, KERN_ERR, LOG_INIT,
						 "2516 Cannot create debugfs "
						"dumpSLIqinfo\n");
				goto debug_failed;
			}
		} else
			phba->debug_dumpSLIqinfo = NULL;
...
lpfc_debugfs_terminate(struct lpfc_vport *vport)
{
	...
	if (atomic_read(&phba->debugfs_vport_count) == 0) {
		...
		if (phba->debug_dumpSLIqinfo) {
			debugfs_remove(phba->debug_dumpSLIqinfo); /* SLIqinfo */
			phba->debug_dumpSLIqinfo = NULL;
		}
		if (phba->debug_dumpData) {
