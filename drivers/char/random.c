 ...
 * at any point prior.
 */
void get_random_bytes(void *buf, size_t _nbytes)
{
	static void *previous;
	/* KABI sanity - make sure upper half is disregarded */
	size_t nbytes = (_nbytes & 0xffffffff);
...
static ssize_t get_random_bytes_user(void __user *buf, size_t nbytes)
{
	...
	for (;;) {
		...
		if (fips_enabled || fips_random) {
			...
			rc = crypto_rng_get_bytes(drbg_random, output, len);
			if (rc < 0) {
				ret = rc;
				goto out;
			}
...
static void add_timer_randomness(struct timer_rand_state *state, unsigned int num)
{
	 ...
	 */
	if (in_irq())
		atomic_add(max(1u, bits * 64) - 1, &(this_cpu_ptr(&irq_randomness)->count));
	else
