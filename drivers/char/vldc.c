...
static void vldc_event(void *arg, int event)
{
	 ...
	 *  window (while interrupts are disabled), attemps to read/write
...
static int vldc_probe(struct vio_dev *vdev, const struct vio_device_id *vio_did)
{
	...
	id = mdesc_get_property(hp, vdev->mp, "id", NULL);
	if (id == NULL) {
	...
	valstr = mdesc_get_property(hp, vdev->mp, "vldc-svc-name", &slen);
	if (valstr == NULL) {
