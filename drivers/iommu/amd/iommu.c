...
static void clear_dte_entry(u16 devid)
{
	...
	if (!amd_iommu_snp_en)
		amd_iommu_dev_table[devid].data[0]  |= DTE_FLAG_TV;
...
static void set_dte_irq_entry(u16 devid, struct irq_remap_table *table)
{
	...
	dte	|= iommu_get_int_tablen(dev_data);
...
static struct irq_remap_table *__alloc_irq_table(int nid, int order)
{
	...
	page = alloc_pages_node(nid, GFP_KERNEL | __GFP_ZERO, order);
	if (!page) {
		kfree(table);
		return NULL;
	}
	table->table = page_address(page);
					       ...
					       unsigned int max_irqs)
{
	...
	nid = iommu->dev ? dev_to_node(&iommu->dev->dev) : NUMA_NO_NODE;
...
out_unlock:
	...
	if (new_table) {
		__free_pages(virt_to_page(new_table->table), order);
		kfree(new_table);
	}
	return table;
}
...
static int alloc_irq_index(u16 devid, int count, bool align,
			   struct pci_dev *pdev, unsigned long max_irqs)
{
	struct irq_remap_table *table;
	...
	table = alloc_irq_table(devid, pdev, max_irqs);
	if (!table)
		return -ENODEV;
			       ...
			       unsigned int nr_irqs, void *arg)
{
	...
	if (info->type == X86_IRQ_ALLOC_TYPE_IOAPIC) {
		...
		table = alloc_irq_table(devid, NULL, max_irqs);
		if (table) {
			if (!table->min_index) {
				/*
		...
		index = alloc_irq_index(devid, nr_irqs, false, NULL, max_irqs);
	}
