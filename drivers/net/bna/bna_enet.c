...
bna_bfi_stats_get_rsp(struct bna *bna, struct bfi_msgq_mhdr *msghdr)
{
	...
	uint64_t *stats_src;
	uint64_t *stats_dst;
	uint32_t tx_enet_mask = ntohl(stats_req->tx_enet_mask);
	uint32_t rx_enet_mask = ntohl(stats_req->rx_enet_mask);
	int count;
	int i;
...
static void
bna_stats_mod_init(struct bna_stats_mod *stats_mod, struct bna *bna)
{
	stats_mod->bna = bna;
}

static void
bna_stats_mod_uninit(struct bna_stats_mod *stats_mod)
{
	stats_mod->bna = NULL;
}

static void
bna_stats_mod_start(struct bna_stats_mod *stats_mod)
{
	stats_mod->ioc_ready = true;
}

static void
bna_ioceth_cb_stats_mod_stopped(struct bna_ioceth *ioceth)
{
	bfa_wc_down(&ioceth->chld_stop_wc);
}

static void
bna_stats_mod_stop(struct bna_stats_mod *stats_mod)
{
	stats_mod->ioc_ready = false;
...
static void
bna_stats_mod_fail(struct bna_stats_mod *stats_mod)
{
	stats_mod->ioc_ready = false;
	if (stats_mod->stats_get_busy)
		bnad_cb_stats_get(stats_mod->bna->bnad, BNA_CB_FAIL,
			&stats_mod->bna->stats);
	stats_mod->stats_get_busy = false;
	stats_mod->stats_clr_busy = false;
...
void
bna_bfi_stats_clr_rsp(struct bna *bna, struct bfi_msgq_mhdr *msghdr)
{
	call_stats_mod_stop_cbfn(&bna->stats_mod);
}
...
bfa_fsm_state_decl(bna_ioceth, last_resp_wait, struct bna_ioceth,
			enum bna_ioceth_event);
bfa_fsm_state_decl(bna_ioceth, chld_stop_wait, struct bna_ioceth,
			enum bna_ioceth_event);
...
static void
bna_ioceth_sm_chld_stop_wait_entry(struct bna_ioceth *ioceth)
{
	bna_ioceth_chld_stop(ioceth);
}

static void
bna_ioceth_sm_chld_stop_wait(struct bna_ioceth *ioceth,
				enum bna_ioceth_event event)
{
	switch (event) {
	case IOCETH_E_IOC_FAILED:
...
{
	struct bna_ioceth *ioceth = (struct bna_ioceth *)cbarg;
