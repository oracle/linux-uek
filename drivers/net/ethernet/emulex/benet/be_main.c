					   ...
					   bool *skip_hw_vlan)
{
	...
	/* Lancer, SH-R ASICs have a bug wherein Packets that are
	 * 32 bytes or less may cause a transmit stall on that port.
	 * So the work-around is to pad short packets (<= 32 bytes)
	 * to a 36-byte length.
	 */
