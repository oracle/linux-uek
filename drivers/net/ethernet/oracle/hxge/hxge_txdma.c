...
static int hxge_init_tx_channel(struct hxge_adapter *hxgep, int channel)
{
	...
	tx_ring->stats.descs_avail = num_tdrs;
	tx_ring->tail = 0;
		...
		struct tx_ring_t *tx_ring, tdc_tdr_head_t head_reg)
{
	...
	tx_ring->stats.descs_avail = atomic_read(&tx_ring->descs_avail);
...
{
        init_waitqueue_head(&tx_ring->reclaim_event);
	tx_ring->thread_pid = kthread_run(reclaim_thread, tx_ring,
				"tx_reclaim/%d", tx_ring->tdc);
	if (IS_ERR(tx_ring->thread_pid)) {
                HXGE_ERR_PRINT("Failed to start kernel thread");
                return -1;
	}
        HXGE_DBG_PRINT("reclaim thread %d started", tx_ring->tdc);
...
static int stop_reclaim_thread(struct tx_ring_t *tx_ring)
{
	kthread_stop(tx_ring->thread_pid);
	tx_ring->thread_pid = NULL;
        HXGE_DBG_PRINT("reclaim thread %d killed", tx_ring->tdc);
        return 0;
}
...

{
	...
	/* HACK: if low on tx descriptors, just return BUSY now */

	if (atomic_read(&tx_ring->descs_avail)<  20) { /* Small Round Number */
		if (!netif_queue_stopped(netdev))
			netif_stop_queue(netdev);
		spin_unlock(&tx_ring->lock);
		tx_ring->stats.descs_busy++;     /* Want to watch this */
		udelay(345);                     /* Arbitrary if empirical */
	...
	if (desc_required == 1) /* small packet */  {
		...
		{
			HXGE_DBG(hxgep, "map_skbdata_to_descs failed");
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
			...
			char *dat =
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 13)
					page_address(fragp->page)
#else
					page_address(fragp->page.p)
#endif
						+ fragp->page_offset;

			HXGE_DBG(hxgep, "Frag %d @ (%p,%d), len %d",i,
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 13)
				(char *)fragp->page
#else
				(char *)fragp->page.p
#endif
					, fragp->page_offset,fragp->size);
	...
	if (atomic_read(&tx_ring->descs_avail) < 0) {
	    HXGE_ERR(hxgep, "Descriptors available less than zero (0x%x)!!",atomic_read(&tx_ring->descs_avail));
	}
...
#else
		char *data = page_address(fragp->page.p);
#endif
