...
static int __sriov_enable(struct pci_dev *pdev, int num_vfs)
{
	...
	err = sdp_get_available_rsrcs(sdp);
	if (err < 0) {
			   ...
			   struct sdp_dev *sdp)
{
	...
	if (num_val != sdp->num_sdp_pfs) {
		dev_err(sdev, "error: num-sdp-vfs length %d is not same as number of SDP PFs %d\n",
	...
	num_val = len / sizeof(u32);
	if (num_val != sdp->num_sdp_pfs) {
		dev_err(sdev, "error: num-sdp-vf-rings length %d is not same as number of SDP PFs %d\n",
			num_val, sdp->num_sdp_pfs);
		return -EINVAL;
	}
