...
#ifdef CONFIG_OCTEONTX2_REMOTE_PCI_CONSOLE_DEBUG
#  define dbgmsg(dev, ...) dev_info((dev), __VA_ARGS__)
#else
#  define dbgmsg(dev, ...) (void)(dev)
#endif // CONFIG_OCTEONTX2_REMOTE_PCI_CONSOLE_DEBUG

#define INVALID_SID ((u32)-1)
static u32 host_sid = 0x030000; /* PEM0, bus 0, dev 0, func 0 */
module_param(host_sid, int, 0644);
MODULE_PARM_DESC(host_sid, "host STREAMID (for IOMMU)");
...
/*
 * This is used to initialize the SMMU to enable host access to the console
 * memory.
 *
 * NOTE: This is to be replaced with an alternate mechanism.
 */
static int pci_console_nexus_init_smmu_host_access(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct pci_console_nexus *pci_cons_nexus = platform_get_drvdata(pdev);
	...
	dbgmsg(dev, "OLD dev->iommu_fwspec %p\n", dev->iommu_fwspec);
	...
	dbgmsg(dev, "NEW dev->iommu_fwspec %p\n", dev->iommu_fwspec);
	...
	pci_cons_nexus->iommu.host_domain = host_domain;
	pci_cons_nexus->iommu.smmu_ops = smmu_ops;
	pci_cons_nexus->iommu.host_sid = host_sid;
 ...
 * 'pci_console_nexus_init_smmu_host_access'.
 */
static int
pci_console_nexus_de_init_smmu_host_access(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct pci_console_nexus *pci_cons_nexus = platform_get_drvdata(pdev);
	const struct iommu_ops *smmu_ops;
	...
	smmu_ops = pci_cons_nexus->iommu.smmu_ops;

	if (smmu_ops && pci_cons_nexus->iommu.host_domain) {
		/* remove device from domain */
		smmu_ops->remove_device(dev);
		/* remove domain */
		smmu_ops->domain_free(
				pci_cons_nexus->iommu.host_domain);

		pci_cons_nexus->iommu.host_domain = NULL;
		pci_cons_nexus->iommu.smmu_ops = NULL;
	}
...
static int pci_console_nexus_init(struct platform_device *pdev)
{
	...
	if (!pci_console_nexus_check_ver(nexus->major_version,
					 nexus->minor_version)) {
		dev_err(dev,
			"Unsupported nexus version %u.%u (%u.%u)\n)",
			nexus->major_version, nexus->minor_version,
			OCTEONTX_PCIE_CONSOLE_MAJOR,
			OCTEONTX_PCIE_CONSOLE_MINOR);
		goto exit;
	}

	if (!nexus->num_consoles) {
	...
	if (num_consoles < nexus->num_consoles) {
		dev_err(dev,
			"Console count mismatch: DT %d, nexus: %d\n",
			num_consoles, nexus->num_consoles);
		goto exit;
	}
...
static int pci_console_nexus_probe(struct platform_device *pdev)
{
	...
	ret = pci_console_nexus_init_smmu_host_access(pdev);
	if (ret)
		goto exit;
...
exit:
	if (ret) {
		...
		pci_console_nexus_de_init_smmu_host_access(pdev);
...
MODULE_DESCRIPTION("OcteonTX PCI Remote Console Driver");
				 ...
				 u64 cons_addr, u64 cons_size)
{
	...
	if (((u32)le32_to_cpu(ring_descr->input_read_index) >=
	     le32_to_cpu(ring_descr->input_buf_size)) ||
	    ((u32)le32_to_cpu(ring_descr->input_write_index) >=
	     le32_to_cpu(ring_descr->input_buf_size)) ||
	    ((u32)le32_to_cpu(ring_descr->output_read_index) >=
	     le32_to_cpu(ring_descr->output_buf_size)) ||
	    ((u32)le32_to_cpu(ring_descr->output_write_index) >=
	     le32_to_cpu(ring_descr->output_buf_size)) ||
	    !ring_descr->input_buf_size ||
	    !ring_descr->output_buf_size ||
	    !ring_descr->input_base_addr ||
	    !ring_descr->output_base_addr) {
	...
	pci_cons->input_ring = ioremap_wc(ring_descr->input_base_addr,
					  ring_descr->input_buf_size);

	/* map the output buffer */
	pci_cons->output_ring = ioremap_wc(ring_descr->output_base_addr,
					   ring_descr->output_buf_size);
	...
	spin_lock_init(&ring_descr->excl_lock);
...
#define CONSOLE_NEXUS_IN_USE_TIMEOUT_USECS 100
	...
	do {
		old_use_mask = le32_to_cpu(nexus_desc->in_use) |
			       (u64)le32_to_cpu(READ_ONCE(
				     ...
				     size_t bytes_to_clear)
{
	...
	if (le32_to_cpu(READ_ONCE(console->host_console_connected)))
		return -1;

	out_wr_idx = le32_to_cpu(READ_ONCE(console->output_write_index));
	out_rd_idx = le32_to_cpu(READ_ONCE(console->output_read_index));
		       ...
		       u8 __iomem *output_ring)
{
	...
	spin_lock(&ring_descr->excl_lock);

	sz = le32_to_cpu(ring_descr->output_buf_size);
	src = buf;
	srclen = len;
	written = 0;
	wait_usecs = 0;

	wr_idx = le32_to_cpu(READ_ONCE(ring_descr->output_write_index));

	while (srclen > 0) {
		rd_idx = le32_to_cpu(READ_ONCE(ring_descr->output_read_index));
		...
		if (avail > 0) {
			...
			if (wr_len)
				memcpy(output_ring + wr_idx, src, wr_len);
			src += wr_len;
			written += wr_len;
			 ...
			 */
			wmb();
			WRITE_ONCE(ring_descr->output_write_index,
				   cpu_to_le32(wr_idx));
		} else if (!avail) {
	...
	spin_unlock(&ring_descr->excl_lock);
...
static int pci_console_init(struct platform_device *pdev)
{
	...
	if (cons_addr !=
	    le64_to_cpu(nexus_desc->console_addr[cons_num])) {
		dev_err(dev,
	...
	if (le32_to_cpu(nexus_desc->in_use) & (1 << cons_num)) {
...
void pci_console_dev_tty_poll(unsigned long ctx)
{
	...
	sz = le32_to_cpu(ring_descr->input_buf_size);
	rd_idx = le32_to_cpu(READ_ONCE(ring_descr->input_read_index));
	wr_idx = le32_to_cpu(READ_ONCE(ring_descr->input_write_index));
	 ...
	 */
	mb();
	WRITE_ONCE(ring_descr->input_read_index, cpu_to_le32(rd_idx));
